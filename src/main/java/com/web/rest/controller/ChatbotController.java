...
// DTOs (clases anidadas, sin cambios)
...
class GeminiRequest {
    ...
    public GeminiRequest(String text) { this.contents = Collections.singletonList(new Content(text)); }
...
public class ChatbotController {
    ...
    // --- CAMBIO AQUÍ: Inyectamos ambas propiedades por separado ---
    ...
    public ResponseEntity<ChatResponseDTO> askGemini(@Valid @RequestBody ChatRequestDTO request) {
        ...
        // --- CAMBIO AQUÍ: Construimos la URL completa ---
        String apiUrl = geminiApiBaseUrl + "?key=" + geminiApiKey;
        // ---------------------------------------------

        try {
            ResponseEntity<GeminiResponse> response = restTemplate.exchange(
                    apiUrl, // Usamos la URL construida
                    HttpMethod.POST,
                    entity,
